...
export default {
  ...
  created () {
    ...
    const index = this.localOptions.findIndex(({ id, path }) => ((path || {}).name === (this.$route || {}).name) || id === this.value)

    if (index > -1) {
      this.currSelected = this.prevSelected = index
  ...
  methods: {
    handleButtonClick (button, index) {
      if (index === this.currSelected) return
